//! ISO 32000 Compliance Report Generator - Final Version
//!
//! Generates a professional PDF report with refined visuals
...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    println!("Generating ISO 32000 Compliance Report (Final)...");
...
// Helper for light gray text (for dark backgrounds)
fn setup_light_text(page: &mut Page) {
    page.graphics()
        .save_state()
...
fn create_pdf_report(
    ...
    // White text inside circle - properly centered
    // For "17.8%" at size 48: 5 characters * ~28px/char = ~140px width
    // Font baseline offset is approximately 1/3 of font size = 16px
    let percentage_text = format!("{:.1}%", compliance);
    let char_width = 28.0; // Approximate width per character for 48pt bold
    let text_width = percentage_text.len() as f64 * char_width;
    let text_x = circle_x - (text_width / 2.0); // True horizontal center
    let text_y = circle_y - 16.0; // Vertical center accounting for baseline

    // WORKAROUND: Since text color seems to always be black, let's use a white rectangle
    // behind the text and then punch out the text shape (knockout effect)
    // First, let's try a different approach - use graphics to draw the text manually

    // Save the graphics state
    page1.graphics().save_state();

    // For now, let's just write the text normally but document the issue
    page1
        .text()
    ...
    // Add a note about the text color limitation
    // TODO: oxidize-pdf doesn't seem to support text color changes
    // Text is always rendered in black
    ...
    // Missing features section - with white text on red background
    page2
        .graphics()
        .save_state()
        .set_fill_color(Color::rgb(0.85, 0.3, 0.3)) // Darker red for better contrast
        .rectangle(45.0, 280.0, 505.0, 190.0)
        .fill()
        .restore_state();

    // White text on red background
    setup_light_text(&mut page2);
    page2
        .text()
        .set_font(Font::HelveticaBold, 14.0)
        .at(50.0, 450.0)
