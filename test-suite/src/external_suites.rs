...
impl ExternalSuiteManager {
    ...
    pub fn load_vera_pdf(&self) -> Result<Vec<TestPdf>> {
        ...
        if !suite_path.exists() {
            anyhow::bail!(
                "veraPDF corpus not found at {:?}. Run download script first.",
                suite_path
            );
        }
        ...
        if self.config.vera_pdf.test_pdfa {
            pdfs.extend(self.load_vera_pdf_category(
                &suite_path,
                "PDF_A-1a",
                ComplianceLevel::PdfA1a,
            )?);
            pdfs.extend(self.load_vera_pdf_category(
                &suite_path,
                "PDF_A-1b",
                ComplianceLevel::PdfA1b,
            )?);
            pdfs.extend(self.load_vera_pdf_category(
                &suite_path,
                "PDF_A-2a",
                ComplianceLevel::PdfA2a,
            )?);
            pdfs.extend(self.load_vera_pdf_category(
                &suite_path,
                "PDF_A-2b",
                ComplianceLevel::PdfA2b,
            )?);
            pdfs.extend(self.load_vera_pdf_category(
                &suite_path,
                "PDF_A-2u",
                ComplianceLevel::PdfA2u,
            )?);
            pdfs.extend(self.load_vera_pdf_category(
                &suite_path,
                "PDF_A-3a",
                ComplianceLevel::PdfA3a,
            )?);
            pdfs.extend(self.load_vera_pdf_category(
                &suite_path,
                "PDF_A-3b",
                ComplianceLevel::PdfA3b,
            )?);
            pdfs.extend(self.load_vera_pdf_category(
                &suite_path,
                "PDF_A-3u",
                ComplianceLevel::PdfA3u,
            )?);
        }
        ...
        for result_type in &["PASS", "FAIL"] {
            ...
            if dir_path.exists() {
                let expected_behavior = if *result_type == "PASS" {
                    ...
                    ExpectedBehavior::CustomValidation(format!(
                        "vera_pdf_{}_fail",
    ...
    pub fn load_qpdf(&self) -> Result<Vec<TestPdf>> {
        ...
        if !suite_path.exists() {
            anyhow::bail!(
                "qpdf test suite not found at {:?}. Run download script first.",
                suite_path
            );
        }
        ...
        for entry in fs::read_dir(dir_path)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("pdf") {
                ...
                let expected_behavior =
                    if file_name.contains("bad") || file_name.contains("invalid") {
    ...
    pub fn load_isartor(&self) -> Result<Vec<TestPdf>> {
        ...
        if !suite_path.exists() {
            anyhow::bail!(
                "Isartor test suite not found at {:?}. Run download script first.",
                suite_path
            );
        }
    ...
    fn get_isartor_test_mapping(&self) -> HashMap<&str, String> {
        ...
        mapping.insert(
            "isartor-6-2-3-3-t01-fail-a",
            "File header: invalid PDF version".to_string(),
        );
        mapping.insert(
            "isartor-6-2-3-3-t02-fail-a",
            "File header: missing %PDF header".to_string(),
        );
        mapping.insert(
            "isartor-6-3-2-t01-fail-a",
            "File trailer: missing %%EOF".to_string(),
        );
        mapping.insert(
            "isartor-6-3-3-1-t01-fail-a",
            "Cross-reference: invalid xref entry".to_string(),
        );
        mapping.insert(
            "isartor-6-4-t01-fail-a",
            "Font: missing required font descriptor".to_string(),
        );
    ...
    pub fn create_download_instructions(&self) -> Result<String> {
        ...
        instructions.push_str(&format!(
            ...
            self.config
                .base_dir
                .join(&self.config.vera_pdf.local_dir)
                .display()
        ));
        ...
        instructions.push_str(&format!(
            ...
            self.config
                .base_dir
                .join(&self.config.qpdf.local_dir)
                .display()
        ));
        ...
        instructions.push_str(&format!(
            ...
            self.base_path
                ...
                .join(&self.config.isartor.local_dir)
                .display()
        ));
