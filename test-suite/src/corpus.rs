...
impl TestCorpus {
    ...
    fn load_fixtures(&mut self) -> Result<()> {
        ...
        self.load_category_fixtures(
            &fixtures_dir.join("valid/edge-cases"),
            TestCategory::EdgeCases,
        )?;

        // Load invalid PDFs
        self.load_category_fixtures(
            &fixtures_dir.join("invalid/corrupted"),
            TestCategory::Corrupted,
        )?;
        self.load_category_fixtures(
            &fixtures_dir.join("invalid/malformed"),
            TestCategory::Malformed,
        )?;
        self.load_category_fixtures(
            &fixtures_dir.join("invalid/security"),
