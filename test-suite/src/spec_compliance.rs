...
impl SpecificationTest for Pdf17ComplianceTester {
    fn test_header_compliance(&self, pdf: &[u8]) -> TestResult {
        ...
        if pdf.len() < 8 {
            return TestResult::fail("PDF 1.7 Header Compliance", "File too small to contain valid header");
        }
        ...
        if let Ok(version) = version_str.trim().parse::<f32>() {
            ...
            return TestResult::fail("PDF 1.7 Header Compliance", "Invalid version number in header");
        }
        ...
        if !found_binary_marker {
            result.add_message("Warning: No binary marker found in header (recommended for PDF 1.7)");
        }
    ...
    fn test_xref_compliance(&self, pdf: &[u8]) -> TestResult {
        ...
        if startxref_pos.is_none() {
            return TestResult::fail("PDF 1.7 Cross-Reference Table Compliance", "Missing startxref keyword");
        }
        ...
        if xref_pos.is_none() {
            ...
            if pdf_str.contains("/Type /XRef") || pdf_str.contains("/Type/XRef") {
                ...
                return TestResult::fail("PDF 1.7 Cross-Reference Table Compliance", "No cross-reference table found");
            }
        } else {
            ...
            if lines.len() < 2 {
                return TestResult::fail("PDF 1.7 Cross-Reference Table Compliance", "Invalid xref table format");
            }
    ...
    fn test_object_compliance(&self, pdf: &[u8]) -> TestResult {
        ...
        if obj_count != endobj_count {
            result.passed = false;
            result.add_message(&format!("Unbalanced obj/endobj: {} obj, {} endobj", obj_count, endobj_count));
        }
    ...
    fn test_content_stream_compliance(&self, pdf: &[u8]) -> TestResult {
        ...
        // Count streams
        let stream_count = pdf_str.matches("stream\n").count() + pdf_str.matches("stream\r\n").count();
        ...
        if stream_count != endstream_count {
            result.passed = false;
            result.add_message(&format!("Unbalanced stream/endstream: {} stream, {} endstream", 
    ...
    fn test_structure_compliance(&self, pdf: &[u8]) -> TestResult {
        ...
        let xref_pos = pdf_str.rfind("xref").or_else(|| pdf_str.find("/Type /XRef"));
...
impl SpecificationTest for Pdf20ComplianceTester {
    ...
    fn test_content_stream_compliance(&self, pdf: &[u8]) -> TestResult {
        ...
        for op in deprecated_ops {
            if pdf_str.contains(&format!(" {}", op)) || pdf_str.contains(&format!("\n{}", op)) {
                result.add_message(&format!("Warning: Uses deprecated operator '{}' in PDF 2.0", op));
            }
        }
