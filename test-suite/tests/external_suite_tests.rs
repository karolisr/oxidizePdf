...
fn test_vera_pdf_corpus() {
    ...
    let pdfs = manager.load_vera_pdf().expect("Failed to load veraPDF corpus");
    assert!(!pdfs.is_empty(), "veraPDF corpus should contain test PDFs");
    
    // Check that we have both PASS and FAIL tests
    let pass_count = pdfs.iter()
        .filter(|pdf| matches!(&pdf.expected_behavior, 
            oxidize_pdf_test_suite::corpus::ExpectedBehavior::ParseSuccess { .. }))
        .count();
    let fail_count = pdfs.len() - pass_count;
    
    println!("veraPDF corpus: {} total PDFs ({} pass, {} fail)", pdfs.len(), pass_count, fail_count);
...
fn test_qpdf_suite() {
    ...
    let bad_pdfs = pdfs.iter()
        .filter(|pdf| pdf.path.file_name()
            .and_then(|n| n.to_str())
            .map(|n| n.contains("bad") || n.contains("invalid"))
            .unwrap_or(false))
        .count();
    
    println!("Found {} PDFs with 'bad' or 'invalid' in filename", bad_pdfs);
}
...
fn test_isartor_suite() {
    ...
    let pdfs = manager.load_isartor().expect("Failed to load Isartor test suite");
    assert!(!pdfs.is_empty(), "Isartor test suite should contain test PDFs");
    
    println!("Isartor test suite: {} PDFs loaded", pdfs.len());
    
    // All Isartor tests should be for PDF/A-1b compliance
    for pdf in &pdfs {
        assert!(pdf.metadata.compliance.iter()
...
fn test_suite_availability() {
    ...
    println!("  veraPDF corpus: {}", 
        if manager.is_suite_available(ExternalSuite::VeraPDF) { "✓ Available" } else { "✗ Not found" });
    println!("  qpdf test suite: {}", 
        if manager.is_suite_available(ExternalSuite::QPdf) { "✓ Available" } else { "✗ Not found" });
    println!("  Isartor test suite: {}", 
        if manager.is_suite_available(ExternalSuite::Isartor) { "✓ Available" } else { "✗ Not found" });
    println!("  PDF Association: {}", 
        if manager.is_suite_available(ExternalSuite::PdfAssociation) { "✓ Available" } else { "✗ Not found" });
    ...
    if !manager.is_suite_available(ExternalSuite::VeraPDF) 
        ...
        let instructions = manager.create_download_instructions()
        ...
        println!("\nDownload instructions saved to: {}", instructions_path.display());
