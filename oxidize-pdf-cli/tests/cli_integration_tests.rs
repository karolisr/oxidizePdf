...
fn assert_pdf_exists_and_valid(path: &Path) {
    ...
    assert!(metadata.len() > 100, "PDF file should have content (> 100 bytes)");
    
    // Basic PDF header check
    let content = fs::read(path).expect("Failed to read PDF file");
    assert!(content.starts_with(b"%PDF-"), "File should start with PDF header");
}
...
fn test_cli_create_command() {
    ...
    assert!(stdout.contains("PDF created successfully"), "Should show success message");
}
...
fn test_cli_demo_command() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Demo PDF created successfully"), "Should show success message");
}
...
fn test_cli_merge_command_not_implemented() {
    ...
    assert!(stderr.contains("PDF merge functionality coming"), "Should show not implemented message");
}
...
fn test_cli_split_command_not_implemented() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("PDF split functionality coming"), "Should show not implemented message");
}
...
fn test_cli_info_command_with_created_pdf() {
    ...
    if info_output.status.success() {
        ...
        assert!(stdout.contains("PDF Information"), "Should show PDF information");
        ...
        assert!(stderr.contains("PDF parser is currently in early development"), 
...
fn test_cli_info_command_detailed() {
    ...
    if info_output.status.success() {
        let stdout = String::from_utf8_lossy(&info_output.stdout);
        assert!(stdout.contains("Detailed Information"), "Should show detailed section");
    }
...
fn test_cli_rotate_command_invalid_angle() {
    ...
    assert!(!output.status.success(), "Command should fail for invalid angle");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Valid angles are 90, 180, 270"), "Should show valid angles");
}
...
fn test_cli_rotate_command_invalid_page_range() {
    ...
    assert!(!output.status.success(), "Command should fail for invalid page range");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Error parsing page range"), "Should show page range error");
}
...
fn test_cli_extract_text_nonexistent_file() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Failed to open PDF"), "Should show file error");
}
...
fn test_cli_extract_text_with_output_file() {
    ...
    if extract_output.status.success() {
        ...
        let stdout = String::from_utf8_lossy(&extract_output.stdout);
        assert!(stdout.contains("Text extracted to"), "Should show success message");
    }
    // If it fails, it's due to text extraction limitations (acceptable for now)
}
...
fn test_cli_missing_required_arguments() {
    ...
    assert!(!output.status.success(), "Command should fail without required args");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("required") || stderr.contains("missing"), "Should show missing argument error");
}