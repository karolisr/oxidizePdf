...
fn create_multipart_merge_request(pdf_files: Vec<Vec<u8>>) -> Request<Body> {
    ...
    for (i, pdf_data) in pdf_files.iter().enumerate() {
        write!(body, "--{}\r\n", boundary).unwrap();
        write!(body, "Content-Disposition: form-data; name=\"files\"; filename=\"test{}.pdf\"\r\n", i + 1).unwrap();
    ...
    // Add options
    write!(body, "--{}\r\n", boundary).unwrap();
    write!(body, "Content-Disposition: form-data; name=\"options\"\r\n\r\n").unwrap();
    let options = json!({
...
async fn test_merge_endpoint_success() {
    ...
    assert!(headers.get("content-disposition").unwrap().to_str().unwrap().contains("merged.pdf"));
...
async fn test_merge_endpoint_insufficient_files() {
    ...
    assert!(error_response["error"].as_str().unwrap().contains("At least 2 PDF files are required"));
}
