...
use oxidize_pdf::text::ocr::{
    MockOcrProvider, OcrOptions, OcrProvider, ImagePreprocessing,
};
...
fn benchmark_mock_ocr_provider(c: &mut Criterion) {
    ...
    c.bench_function("mock_ocr_basic", |b| {
        b.iter(|| {
            provider.process_image(black_box(&image_data), black_box(&options))
        })
    });
    ...
    for delay in [0, 50, 100, 200].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("delay_ms", delay),
            delay,
            |b, _| {
                b.iter(|| {
                    custom_provider.process_image(black_box(&image_data), black_box(&options))
                })
            },
        );
    }
...
fn benchmark_mock_ocr_image_sizes(c: &mut Criterion) {
    ...
    group.bench_function("small_image", |b| {
        b.iter(|| {
            provider.process_image(black_box(&small_image), black_box(&options))
        })
    });
    
    group.bench_function("large_image", |b| {
        b.iter(|| {
            provider.process_image(black_box(&large_image), black_box(&options))
        })
    });
...
fn benchmark_mock_ocr_image_formats(c: &mut Criterion) {
    ...
    group.bench_function("jpeg_format", |b| {
        b.iter(|| {
            provider.process_image(black_box(&jpeg_data), black_box(&options))
        })
    });
    
    group.bench_function("png_format", |b| {
        b.iter(|| {
            provider.process_image(black_box(&png_data), black_box(&options))
        })
    });
...
fn benchmark_ocr_options_impact(c: &mut Criterion) {
    ...
    group.bench_function("default_options", |b| {
        b.iter(|| {
            provider.process_image(black_box(&image_data), black_box(&default_options))
        })
    });
    ...
    group.bench_function("high_preprocessing", |b| {
        b.iter(|| {
            provider.process_image(black_box(&image_data), black_box(&high_preprocessing_options))
        })
    });
...
fn benchmark_provider_comparison(c: &mut Criterion) {
    ...
    group.bench_function("mock_provider", |b| {
        b.iter(|| {
            mock_provider.process_image(black_box(&image_data), black_box(&options))
        })
    });
    ...
    #[cfg(feature = "ocr-tesseract")]
    {
        if let Ok(tesseract_provider) = TesseractOcrProvider::new() {
            group.bench_function("tesseract_provider", |b| {
                b.iter(|| {
                    let _ = tesseract_provider.process_image(black_box(&image_data), black_box(&options));
                })
            });
        }
    }
