...
use oxidize_pdf::{
    ...
    batch_split_pdfs, batch_merge_pdfs,
    Document, Page, Font, Color,
};
use std::fs;
...
fn example_batch_split() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let options = BatchOptions::default()
        ...
        .with_progress_callback(|info| {
            println!("  Split progress: {:.1}% - {} completed, {} failed", 
                info.percentage(),
                info.completed_jobs,
                info.failed_jobs
            );
    ...
    println!("✓ Split completed: {} successful, {} failed in {:.2}s\n",
        summary.successful,
        summary.failed,
...
fn example_batch_merge() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✓ Merge completed: {} successful in {:.2}s\n",
        summary.successful,
        summary.duration.as_secs_f64()
    );
...
fn example_custom_batch() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let options = BatchOptions::default()
        ...
        .with_progress_callback({
            ...
            move |info| {
                ...
                println!("  Progress: {:.1}% | Jobs: {}/{} | Pages: {}/{} | ETA: {}",
                    info.percentage(),
                    info.completed_jobs,
                    info.total_jobs,
                    pages_done,
    ...
    println!("✓ Custom batch completed: {} jobs in {:.2}s\n",
        summary.successful,
        summary.duration.as_secs_f64()
    );
...
fn example_mixed_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let options = BatchOptions::default()
        ...
        .with_progress_callback(|info| {
            ...
            print!("] {:.1}% - {:.1} jobs/s", info.percentage(), info.throughput);
            use std::io::Write;
    ...
    println!("  - Total time with parallelism: {:.2}s", elapsed.as_secs_f64());
