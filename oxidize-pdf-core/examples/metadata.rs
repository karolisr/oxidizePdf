...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    page.text()
        ...
        .at(50.0, 670.0)
        .write(&format!("Created: {}", creation_date.format("%B %d, %Y at %H:%M UTC")))?
        .at(50.0, 640.0)
        .write(&format!("Modified: {}", local_mod_date.format("%B %d, %Y at %H:%M %Z")))?;
    ...
    println!("  Creation Date: {}", creation_date.format("%Y-%m-%d %H:%M:%S UTC"));
