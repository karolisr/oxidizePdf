...
use oxidize_pdf::streaming::{
    StreamingDocument, StreamingOptions, ChunkProcessor, ChunkOptions,
    IncrementalParser, ParseEvent, TextStreamer, TextStreamOptions,
...
fn demo_chunk_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    process_in_chunks(cursor, options, |chunk| {
        match chunk.chunk_type {
            ChunkType::Text => {
                text_chunks += 1;
                println!("  Text chunk: {} bytes at position {}", 
            ...
            ChunkType::Graphics => {
                graphics_chunks += 1;
                println!("  Graphics chunk: {} bytes at position {}", 
                    chunk.size, chunk.position);
            }
            _ => {}
        }
