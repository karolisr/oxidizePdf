...
fn main() -> Result<()> {
    ...
    println!("   ✅ SUCCESS: Generated {} bytes in memory", pdf_bytes.len());
    ...
    page.text()
        .set_font(oxidize_pdf::Font::Helvetica, 12.0)
        .at(50.0, 750.0)
        .write("This PDF should be compressed using FlateDecode filter. ".repeat(10).as_str())?;
    ...
    println!("   ✅ SUCCESS: Compressed PDF size: {} bytes", compressed_bytes.len());
    println!("   ✅ SUCCESS: Uncompressed PDF size: {} bytes", uncompressed_bytes.len());
    ...
    if compressed_bytes.len() < uncompressed_bytes.len() {
        ...
        println!("   ✅ SUCCESS: Compression working - saved {} bytes ({:.1}%)", savings, percent);
    } else {
    ...
    println!("   ✅ SUCCESS: Generated clipping test PDF with {} bytes", clip_pdf_bytes.len());
