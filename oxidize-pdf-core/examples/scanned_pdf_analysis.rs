...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, analysis) in analyses.iter().enumerate() {
        ...
        match analysis.page_type {
            PageType::Scanned => println!("   📱 This page appears to be SCANNED - OCR recommended"),
    ...
    if scanned_pages.is_empty() {
        ...
        println!("   📱 Found {} scanned page(s): {:?}", scanned_pages.len(), scanned_pages);
            ...
            match analyzer_with_options.extract_text_from_scanned_page(page_num, &ocr_provider) {
                Ok(ocr_result) => {
                    ...
                    for (i, fragment) in ocr_result.fragments.iter().take(3).enumerate() {
                        println!("        Fragment {}: \"{}\" at ({:.1}, {:.1}) - {:.1}% confidence",
                                i + 1, fragment.text, fragment.x, fragment.y, fragment.confidence * 100.0);
                    }
                }
                Err(e) => {
    ...
    if bulk_results.is_empty() {
        ...
        for (page_num, ocr_result) in bulk_results {
            println!("      Page {}: {} chars, {:.1}% confidence", 
                    page_num + 1, ocr_result.text.len(), ocr_result.confidence * 100.0);
        }
    }
    ...
    for i in 0..analyses.len() {
        ...
        println!("   Page {}: {} (dominant content: {:.1}%)", 
                i + 1, 
                if is_scanned { "📱 Scanned" } else { "📝 Text/Mixed" },
...
fn create_sample_pdf() -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
    ...
    page1.text()
        ...
        .at(50.0, 700.0)
        .write("This page contains substantial text content that would be classified as vector text.")?;
