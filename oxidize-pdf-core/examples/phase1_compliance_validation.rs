...
fn main() -> Result<()> {
    ...
    if pdf_bytes.len() > 100 && pdf_bytes.starts_with(b"%PDF-") {
        println!("      ✅ SUCCESS: Generated {}-byte PDF in memory", pdf_bytes.len());
    } else {
    ...
    page.text()
        ...
        .at(50.0, 750.0)
        .write("This is a test string for compression analysis. ".repeat(20).as_str())?;
    ...
    if w_success && w_star_success {
        ...
        println!("      ❌ FAILED: Clipping operators not generated (W: {}, W*: {})", w_success, w_star_success);
        return Ok(());
    }
    
    // Generate final validation PDF
    let clip_pdf_bytes = doc_clip.to_bytes()?;  
    println!("      ✅ SUCCESS: Generated clipping validation PDF ({} bytes)", clip_pdf_bytes.len());
