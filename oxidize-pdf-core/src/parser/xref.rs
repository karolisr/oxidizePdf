...
impl XRefTable {
    ...
    pub fn parse<R: Read + Seek>(reader: &mut BufReader<R>) -> ParseResult<Self> {
        ...
        if line.trim() == "xref" {
            ...
            if let Some(stream) = obj.as_stream() {
                ...
                if stream.dict.get("Type").and_then(|o| o.as_name()).map(|n| n.as_str()) == Some("XRef") {
    ...
    fn parse_traditional_xref<R: Read + Seek>(
        ...
        loop {
            ...
            let first_obj_num = parts[0].parse::<u32>()
    ...
    fn parse_xref_entry(line: &str) -> ParseResult<XRefEntry> {
        ...
        let offset = offset_str.trim().parse::<u64>()
            .map_err(|_| ParseError::InvalidXRef)?;
        let generation = gen_str.trim().parse::<u16>()
            .map_err(|_| ParseError::InvalidXRef)?;
...
impl XRefStream {
    ...
    fn decode_entries(&mut self) -> ParseResult<()> {
        ...
        let w: Vec<usize> = w_array.0.iter()
            .map(|obj| obj.as_integer()
                .ok_or_else(|| ParseError::SyntaxError {
                    position: 0,
