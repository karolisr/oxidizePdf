...
impl<R: Read> Lexer<R> {
    ...
    fn read_name(&mut self) -> ParseResult<Token> {
        ...
        while let Some(ch) = self.peek_char()? {
            ...
            if ch == b'#' {
                let hex1 = self.consume_char()?.ok_or_else(|| ParseError::SyntaxError {
                    position: self.position,
                    message: "Incomplete hex code in name".to_string(),
                })?;
                let hex2 = self.consume_char()?.ok_or_else(|| ParseError::SyntaxError {
                    position: self.position,
                    message: "Incomplete hex code in name".to_string(),
                })?;
    ...
    fn read_literal_string(&mut self) -> ParseResult<Token> {
        ...
        while paren_depth > 0 {
            let ch = self.consume_char()?.ok_or_else(|| ParseError::SyntaxError {
                position: self.position,
    ...
    fn read_angle_bracket(&mut self) -> ParseResult<Token> {
        ...
        if self.peek_char()? == Some(b'<') {
            ...
            for chunk in hex_chars.as_bytes().chunks(2) {
                ...
                let byte = u8::from_str_radix(hex_str, 16).map_err(|_| ParseError::SyntaxError {
                    position: self.position,
...
mod tests {
    ...
    fn test_lexer_strings() {
        ...
        assert_eq!(lexer.next_token().unwrap(), Token::String(b"Hello World".to_vec()));
    ...
    fn test_lexer_comments() {
        ...
        assert_eq!(lexer.next_token().unwrap(), Token::Comment("PDF-1.7".to_string()));
        assert_eq!(lexer.next_token().unwrap(), Token::Integer(123));
    }
}