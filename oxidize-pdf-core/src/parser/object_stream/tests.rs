...
fn test_object_stream_parse_invalid_n_type() {
    let mut dict = PdfDictionary::new();
    dict.insert("N".to_string(), PdfObject::Name(PdfName::new("invalid".to_string())));
    dict.insert("First".to_string(), PdfObject::Integer(10));
...
fn test_object_stream_parse_invalid_first_type() {
    let mut dict = PdfDictionary::new();
    dict.insert("N".to_string(), PdfObject::Integer(2));
    dict.insert("First".to_string(), PdfObject::Name(PdfName::new("invalid".to_string())));
...
fn test_xref_entry_type_compressed() {
    ...
    assert_eq!(simple.generation, 0, "Compressed entry should have generation 0");
...
fn test_object_stream_debug() {
    ...
    assert!(debug_str.contains("ObjectStream"), "Debug output should contain struct name");
    assert!(debug_str.contains("n: 1"), "Debug output should show n value");
    assert!(debug_str.contains("first: 10"), "Debug output should show first value");
}
