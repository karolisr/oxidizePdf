...
mod tests {
    ...
    use super::super::{EntityType, EntityMetadata};
    use crate::page::Page;
    ...
    fn test_entity_builder_with_metadata() {
        ...
        assert_eq!(builder.metadata.properties.get("rows"), Some(&"5".to_string()));
        assert_eq!(builder.metadata.properties.get("columns"), Some(&"3".to_string()));
        assert_eq!(builder.metadata.properties.get("header"), Some(&"true".to_string()));
    }
    ...
    fn test_entity_builder_with_schema() {
        let mut page = Page::a4();
        let builder = EntityBuilder::new(&mut page, EntityType::Paragraph, (0.0, 0.0, 300.0, 100.0))
            .with_schema("https://schema.org/Article");
        
        assert_eq!(builder.metadata.schema, Some("https://schema.org/Article".to_string()));
    }
    ...
    fn test_complete_marking_workflow() {
        ...
        // Mark a heading
        let heading_id = marker.mark_text((50.0, 50.0, 500.0, 72.0))
        ...
        // Mark a paragraph
        let para_id = marker.mark_text((50.0, 100.0, 500.0, 200.0))
        ...
        // Mark an image
        let img_id = marker.mark_image((100.0, 250.0, 300.0, 200.0))
    ...
    fn test_marking_different_page_sizes() {
        ...
        for mut page in pages {
            ...
            let id = marker.mark_text((0.0, 0.0, 100.0, 50.0))
    ...
    fn test_marking_all_entity_types() {
        ...
        for (entity_type, bounds) in types {
            let id = marker.mark(entity_type, bounds)
