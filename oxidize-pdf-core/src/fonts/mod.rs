...
pub use loader::{FontLoader, FontData, FontFormat};
pub use ttf_parser::{TtfParser, GlyphMapping};
pub use font_descriptor::{FontDescriptor, FontFlags};
pub use font_metrics::{FontMetrics, TextMeasurement};
pub use font_cache::FontCache;
pub use embedder::{FontEmbedder, EmbeddingOptions, FontEncoding};
...
impl Font {
    ...
    pub fn measure_text(&self, text: &str, font_size: f32) -> TextMeasurement {
        self.metrics.measure_text(text, font_size, &self.glyph_mapping)
    }
...
mod tests {
    ...
    fn test_font_format_detection() {
        ...
        assert!(matches!(FontFormat::detect(&ttf_data), Ok(FontFormat::TrueType)));
        
        // OTF magic bytes
        let otf_data = vec![0x4F, 0x54, 0x54, 0x4F];
        assert!(matches!(FontFormat::detect(&otf_data), Ok(FontFormat::OpenType)));
