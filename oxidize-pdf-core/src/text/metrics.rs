...
mod tests {
    ...
    fn test_helvetica_specific_characters() {
        let chars = [
            ...
            ('a', 556), ('b', 556), ('0', 556), ('1', 556),
            ('@', 1015), ('M', 833), ('W', 944), ('i', 222),
        ];
        
        for (ch, expected_width) in &chars {
            let width = measure_char(*ch, Font::Helvetica, 1000.0);
            let expected = *expected_width as f64;
            assert!((width - expected).abs() < 0.1, "Character '{}' width mismatch: {} vs {}", ch, width, expected);
        }
    }
    ...
    fn test_times_specific_characters() {
        let chars = [
            (' ', 250), ('A', 722), ('B', 667), ('C', 667),
            ('a', 444), ('b', 500), ('0', 500), ('1', 500),
            ('@', 921), ('M', 889), ('W', 944), ('i', 278),
        ];
    ...
    fn test_courier_monospace_property() {
        ...
        for ch in &chars {
            ...
            assert_eq!(width, 600.0, "Courier character '{}' should be 600 units", ch);
        }
    }
    ...
    fn test_font_size_scaling() {
        ...
        for size in &sizes {
            ...
            assert!((width - expected).abs() < 0.01, "Size {} scaling incorrect", size);
        }
    }
    ...
    fn test_measure_text_consistency() {
        ...
        let individual_sum: f64 = text.chars()
    ...
    fn test_unicode_characters_default_width() {
        ...
        for ch in &unicode_chars {
            ...
            assert!((helvetica_width - helvetica_expected).abs() < 0.01, "Helvetica width mismatch");
            assert!((times_width - times_expected).abs() < 0.01, "Times width mismatch");
            assert!((courier_width - courier_expected).abs() < 0.01, "Courier width mismatch");
        }
    }
}
