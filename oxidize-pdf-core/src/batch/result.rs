...
impl JobResult {
    ...
    pub fn duration(&self) -> Option<Duration> {
        match self {
            JobResult::Success { duration, .. } |
            JobResult::Failed { duration, .. } => Some(*duration),
...
impl fmt::Display for JobResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            JobResult::Success { job_name, duration, output_files } => {
                write!(
                    f,
            ...
            JobResult::Failed { job_name, duration, error } => {
                write!(
                    f,
                    "✗ {} - failed after {:.2}s: {}",
                    job_name,
    ...
    pub fn errors(&self) -> Vec<(&str, &str)> {
        self.failed_jobs()
            .filter_map(|r| match r {
                JobResult::Failed { job_name, error, .. } => Some((job_name.as_str(), error.as_str())),
                _ => None,
            })
            .collect()
    }
...
impl BatchSummary {
    ...
    pub fn average_duration(&self) -> Option<Duration> {
        let durations: Vec<_> = self.results
            .iter()
    ...
    pub fn format_report(&self) -> String {
        ...
        // List failed jobs
        let failed_jobs: Vec<_> = self.results
            .iter()
