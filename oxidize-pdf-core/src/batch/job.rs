    ...
    pub fn display_name(&self) -> String {
        match self {
            BatchJob::Split { input, .. } => {
                format!("Split {}", input.file_name().unwrap_or_default().to_string_lossy())
            }
            BatchJob::Merge { inputs, output } => {
                format!("Merge {} files to {}", 
                    inputs.len(), 
                    output.file_name().unwrap_or_default().to_string_lossy())
            }
            BatchJob::Rotate { input, rotation, .. } => {
                format!("Rotate {} by {}°", 
                    input.file_name().unwrap_or_default().to_string_lossy(),
                    rotation)
            }
            BatchJob::Extract { input, pages, .. } => {
                format!("Extract {} pages from {}", 
                    pages.len(),
                    input.file_name().unwrap_or_default().to_string_lossy())
            }
            BatchJob::Compress { input, quality, .. } => {
                format!("Compress {} (quality: {})", 
                    input.file_name().unwrap_or_default().to_string_lossy(),
                    quality)
            }
...
impl fmt::Debug for BatchJob {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            BatchJob::Rotate { input, output, rotation, pages } => {
            ...
            BatchJob::Compress { input, output, quality } => {
