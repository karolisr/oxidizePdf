...
use crate::objects::{Object, Dictionary};
use std::fs::File;
use std::io::Read;
use std::path::Path;
...
impl Image {
    ...
    pub fn to_pdf_object(&self) -> Object {
        ...
        // Bits per component
        dict.set("BitsPerComponent", Object::Integer(self.bits_per_component as i64));
...
fn parse_jpeg_header(data: &[u8]) -> Result<(u32, u32, ColorSpace, u8)> {
    ...
    if width == 0 || height == 0 {
        return Err(PdfError::InvalidImage("Could not find image dimensions".to_string()));
    }
    ...
    let color_space = match components {
        ...
        _ => return Err(PdfError::InvalidImage(format!("Unsupported number of components: {}", components))),
    };
