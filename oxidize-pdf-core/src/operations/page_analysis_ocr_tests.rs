...
mod tests {
    ...
    fn test_ocr_text_fragment_creation() {
        use crate::text::{OcrTextFragment, FragmentType};
    ...
    fn test_engine_options_hashmap() {
        ...
        assert_eq!(options.engine_options.get("tesseract_psm"), Some(&"6".to_string()));
        assert_eq!(options.engine_options.get("tesseract_oem"), Some(&"3".to_string()));
    }
}