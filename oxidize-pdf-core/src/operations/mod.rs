...
impl PageRange {
    ...
    pub fn parse(s: &str) -> Result<Self, OperationError> {
        ...
        // Try list (e.g., "1,3,5")
        if s.contains(',') {
            let pages: Result<Vec<usize>, _> = s.split(',')
                .map(|p| {
                    let page = p.trim().parse::<usize>()
                        .map_err(|_| OperationError::InvalidPageRange(
                            format!("Invalid page: {}", p)
                        ))?;
                    if page == 0 {
                        return Err(OperationError::InvalidPageRange(
                            "Page numbers start at 1".to_string()
        ...
        Err(OperationError::InvalidPageRange(format!("Invalid format: {}", s)))
    }
...
mod tests {
    ...
    fn test_page_range_indices() {
        ...
        assert_eq!(
            PageRange::Single(5).get_indices(total).unwrap(),
            vec![5]
        );
