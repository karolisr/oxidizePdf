...
fn test_array_push_and_len() {
    ...
    assert_eq!(array.len(), 2, "Array should have two elements after second push");
}
...
fn test_array_pop() {
    ...
    assert!(array.pop().is_none(), "Pop on empty array should return None");
    ...
    assert_eq!(array.len(), 0, "Array should be empty after popping all elements");
    assert!(array.is_empty(), "Array should be empty after popping all elements");
}
...
fn test_array_remove() {
    ...
    assert_eq!(array.len(), 2, "Array should have two elements after remove");
...
fn test_array_get() {
    ...
    assert!(array.get(0).is_none(), "Get on empty array should return None");
    ...
    assert!(array.get(2).is_none(), "Get out of bounds should return None");
...
fn test_array_get_mut() {
    ...
    assert!(array.get_mut(0).is_none(), "Get_mut on empty array should return None");
    ...
    assert!(array.get_mut(2).is_none(), "Get_mut out of bounds should return None");
...
fn test_array_clear() {
    ...
    assert_eq!(array.len(), 3, "Array should have three elements before clear");
    
    array.clear();
    
    assert_eq!(array.len(), 0, "Array should be empty after clear");
    assert!(array.is_empty(), "Array should be empty after clear");
    assert!(array.get(0).is_none(), "No elements should be accessible after clear");
}
...
fn test_array_iter() {
    ...
    assert_eq!(empty_collected.len(), 0, "Empty array iterator should yield no elements");
}
...
fn test_array_from_iterator() {
    ...
    assert_eq!(array.len(), 3, "Array from iterator should have correct length");
    
    // Test empty iterator
    let empty_objects: Vec<Object> = vec![];
    let empty_array: Array = empty_objects.into_iter().collect();
    assert_eq!(empty_array.len(), 0, "Array from empty iterator should be empty");
    assert!(empty_array.is_empty(), "Array from empty iterator should be empty");
}
...
fn test_array_large_operations() {
    ...
    assert_eq!(array.len(), 100, "Array should handle large number of elements");
    assert!(array.get(0).is_some(), "First element should exist");
    assert!(array.get(99).is_some(), "Last element should exist");
    assert!(array.get(100).is_none(), "Out of bounds access should return None");
    ...
    assert_eq!(array.len(), 50, "Array should have correct length after removals");
    assert!(array.get(49).is_some(), "Last element should exist");
    assert!(array.get(50).is_none(), "Beyond last element should return None");
}
...
fn test_array_clone() {
    ...
    assert_eq!(cloned.len(), original.len(), "Cloned array should have same length");
...
fn test_array_debug() {
    ...
    assert!(debug_str.contains("Array"), "Debug output should contain Array");
