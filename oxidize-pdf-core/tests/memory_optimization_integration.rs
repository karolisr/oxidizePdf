...
use oxidize_pdf::memory::{MemoryOptions, MemoryManager};
...
fn test_memory_optimized_document_creation() -> Result<()> {
    ...
    let memory_configs = vec![
        ...
        ("custom", MemoryOptions::default().with_cache_size(500).with_lazy_loading(false)),
    ];
    ...
    for (config_name, _memory_options) in memory_configs {
        ...
        for page_num in 1..=5 {
            ...
            page.text()
                ...
                .at(50.0, 750.0)
                .write(&format!("Memory config: {} - Page {}", config_name, page_num))?;
        ...
        println!("Generated PDF size for {}: {} bytes", config_name, file_size);
...
fn test_optimized_reader_memory_workflows() -> Result<()> {
    ...
    for (i, memory_options) in memory_configs.into_iter().enumerate() {
        ...
        let reader_result = OptimizedPdfReader::new_with_options(
            cursor, 
            parse_options, 
            memory_options.clone()
        );
        ...
        if reader_result.is_ok() {
            ...
            println!("Optimized reader creation failed for config {} (expected)", i);
        }
    }
    ...
    for page_num in 1..=page_count {
        ...
        for section in 0..5 {
            ...
            for line in 0..8 {
                ...
                if y_pos > 50.0 { // Stay within page bounds
                    page.text()
                        .set_font(Font::Helvetica, 10.0)
                        .at(70.0, y_pos)
                        .write(&format!("Content line {} of section {}", line + 1, section + 1))?;
                }
            }
        }
        ...
        if page_num % 5 == 0 {
            ...
            println!("Generated {} pages, current size: {} bytes", page_num, partial_bytes.len());
        }
    }
...
fn test_content_type_memory_optimization() -> Result<()> {
        ...
        for i in 0..50 {
            ...
            if y_pos > 50.0 {
                page.text()
                    .set_font(Font::Helvetica, 10.0)
                    .at(50.0, y_pos)
                    .write(&format!("Text line {} with substantial content for memory testing", i + 1))?;
            }
        }
        Ok(())
    }
    ...
    for (content_type, content_fn) in content_types {
        ...
        let file_path = temp_dir.path().join(format!("memory_content_{}.pdf", content_type));
...
fn test_memory_statistics_tracking() -> Result<()> {
    ...
    assert_eq!(final_stats.cache_hits + final_stats.cache_misses, page_count as usize);
