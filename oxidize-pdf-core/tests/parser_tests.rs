...
#[cfg(test)]
mod tests {
    use oxidize_pdf_core::parser::{PdfReader, lexer::*, objects::*, header::*, xref::*, trailer::*};
    use std::io::Cursor;
...
%%EOF";
    ...
    fn test_lexer_tokenization() {
        ...
        assert!(matches!(lexer.next_token().unwrap(), Token::Reference(2, 0)));
    ...
    fn test_object_parsing() {
        ...
        assert!(matches!(dict.get("Pages"), Some(PdfObject::Reference(2, 0))));
    }
    ...
    fn test_parse_nested_structures() {
        ...
        assert!(matches!(font_dict.get("F1"), Some(PdfObject::Reference(4, 0))));
    }
