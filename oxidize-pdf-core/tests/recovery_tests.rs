...
use oxidize_pdf::{
    ...
    CorruptionType, detect_corruption,
    RepairStrategy, repair_document,
    ObjectScanner, validate_pdf,
    Document, Page,
};
use std::fs::File;
use std::io::Write;
...
fn test_corruption_detection() {
    ...
    for (filename, corruption_type) in test_cases {
        ...
        assert!(report.severity > 0, "Should detect corruption in {}", filename);
        assert!(!report.errors.is_empty(), "Should have errors for {}", filename);
    }
}
...
fn test_partial_recovery() {
    ...
    let mut recovery = PdfRecovery::new(
        RecoveryOptions::default().with_partial_content(true)
    );
...
fn test_object_scanner() {
    ...
    writeln!(file, "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] >>").unwrap();
    writeln!(file, "endobj").unwrap();
    ...
    println!("Found {} objects, {} pages", result.total_objects, result.estimated_pages);
}
