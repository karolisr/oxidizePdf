...
fn test_pdf_parsing_with_fixtures() {
    ...
    if fixtures_available() {
            ...
            match fs::read(&pdf_path) {
                Ok(pdf_bytes) => {
                    ...
                    if pdf_bytes.len() >= 8 && pdf_bytes.starts_with(b"%PDF-") {
                        successful_tests += 1;
                        println!("  ✅ {} is valid PDF format", 
                               pdf_path.file_name().unwrap().to_string_lossy());
                    } else {
                        println!("  ❌ {} has invalid PDF header", 
                               pdf_path.file_name().unwrap().to_string_lossy());
                    }
                }
                Err(e) => {
                    println!("  🚫 Could not read {}: {}", 
                           pdf_path.file_name().unwrap().to_string_lossy(), e);
                }
            }
        }
        
        println!("📊 Results: {}/{} PDFs passed basic validation ({:.1}% success rate)", 
        ...
        assert!(success_rate >= 0.8, "PDF validation success rate too low: {:.1}%", success_rate * 100.0);
    } else {
...
fn test_fixture_statistics() {
    ...
    if fixtures_available() {
        ...
        // Basic sanity checks
        assert!(stats.total_pdfs > 0, "Should have PDFs if fixtures available");
        assert!(stats.total_size_bytes > 0, "Should have total size if PDFs exist");
...
fn test_pdf_content_analysis() {
    ...
    for pdf_path in fixtures {
        if let Ok(pdf_bytes) = fs::read(&pdf_path) {
            ...
            println!("  📄 {}: {} pages, text: {}, images: {}", 
                   pdf_path.file_name().unwrap().to_string_lossy(),
                   page_count.max(1),
                   if pdf_str.contains("/Font") { "yes" } else { "no" },
                   if pdf_str.contains("/Image") { "yes" } else { "no" });
        }
    }
    
    println!("📊 Content Analysis Results:");
    println!("   Analyzed: {} PDFs", analyzed);
    println!("   With text: {} ({:.1}%)", with_text, with_text as f64 / analyzed as f64 * 100.0);
    println!("   With images: {} ({:.1}%)", with_images, with_images as f64 / analyzed as f64 * 100.0);
    println!("   Multi-page: {} ({:.1}%)", multipage, multipage as f64 / analyzed as f64 * 100.0);
...
fn test_pdf_performance_benchmark() {
    ...
    println!("   Throughput: {:.2} MB/s", (total_bytes as f64 / 1_048_576.0) / elapsed.as_secs_f64());
    println!("   Avg time per PDF: {:.2}ms", elapsed.as_millis() as f64 / successful_reads as f64);
    
    // Performance assertions
    assert!(successful_reads > 0, "Should have successfully read some PDFs");
    assert!(elapsed.as_secs() < 30, "Benchmark should complete within 30 seconds");
}
...
fn test_environment_detection() {
    ...
    if in_ci {
        assert!(!fixtures_available(), "Fixtures should not be available in CI");
        println!("🤖 Correctly detected CI environment");
    }
    
    // Test fixture disable
    std::env::set_var("OXIDIZE_PDF_FIXTURES", "off");
    assert!(!fixtures_available(), "Fixtures should be disabled when OXIDIZE_PDF_FIXTURES=off");
